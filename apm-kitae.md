...
## 커밋 전 브랜치 확인 (습관화 — 절대 규칙)

- 커밋 직전 반드시 `git branch --show-current`로 현재 브랜치를 확인한다
- 특히 **PR 머지·브랜치 전환 직후**에는 로컬이 develop으로 바뀌어 있을 수 있다 — 확인 없이 커밋하면 develop 직행 사고가 난다
- develop/main에 직접 커밋 금지 — 작업 커밋은 이슈 브랜치(`{유형}/#이슈번호`)에서만
